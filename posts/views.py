...
def new_post(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            Post.objects.create(text=text, group=group, author=author, image=image)
...
def post_view(request, username, post_id):
    ...
    post = get_object_or_404(Post, author=user2, pk=post_id)
    count = Post.objects.filter(author=user2).count()
    com = Comment.objects.filter(post=post)
    form = CommentForm()
    return render(request, "post.html", {'user2': user2, 'post': post,'count': count, "com" : com, "form" : form})
...
def post_edit(request, username, post_id):
    ...
    if request.method == 'POST':
        form = PostForm(request.POST or None, files=request.FILES or None, instance=id_post) #, data=request.POST
        if form.is_valid():
            form.save()
            return redirect(post_view, username, post_id)
        else:
